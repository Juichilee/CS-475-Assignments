...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Juichi Lee" };
...
float bestPerformance = 0.0f;
int updatePerformance = 0;
...
Animate( )
{
	...
	if( ShowPerformance )
	{
		...
		totalTimeElapsed = totalTimeElapsed + ElapsedTime;
	}
...
Display( )
{
	...
	{
		char str[128];
		if (totalTimeElapsed > 0.1 && totalTimeElapsed <= 0.75) { // Start recording bestPerformance between 0.25 and 0.75 totalTimeElapsed
			
			float currPerformance = (float)NUM_PARTICLES / ElapsedTime / 1000000.;
			if (currPerformance > bestPerformance) {
				bestPerformance = currPerformance;
			}
		}
		sprintf(str, "%6.1f MegaParticles/Sec", bestPerformance);
